...
echo '<div class="text-center text-primary display-1 h1">Processors
        <hr>
    </div>';
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo '<a href="product_details.php?id='.$row['ProductID'].'"><img src="'.$row['ProductImage'].'" alt="'.$row['ProductName'].'" width="100%" height="200px">';
    echo '<div class="card-body">';
    echo '<h2 class="card-text">'.$row['ProductName'].'</h2>';
    echo '<h2 class="card-text">&#8377; '.$row['ProductPrice'].'</h2>';
    echo '</div>';
    echo '</a>';
    echo '</div>';
